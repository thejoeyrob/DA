(() => {
  ...
  const TEXT = {
    en:{
      ...
      completeEyebrow:'ASSESSMENT COMPLETE',completeTitle:'Thank you for taking part',completeLead:'Your responses have been converted into the result code below.',copyCode:'Copy result code',resultInstruction:'Save this code and provide it to the Danco administrator or hiring contact. Your score and private work-style guide are not shown on this screen.',nextApplicant:'Start next applicant',help:'Help',helpTitle:'Need a hand?',helpDefault:'Follow the instruction shown at the top of the current screen. During the timed assessment, review first and select one answer when the cards become active.',repeatAudio:'Repeat spoken audio',repeatAudioHint:'Hear the current instruction or question again.',voiceLabel:'Narration voice',voiceHint:'English uses only an Enhanced/Premium American male voice. Spanish automatically uses the original preferred female voice.',voiceReady:'High-quality narration voice ready.',voiceInstallRequired:'High-quality American male voice not installed. On iPhone: Settings > Accessibility > Read & Speak > Voices > English > English (US), then download an Enhanced/Premium male voice and reopen the app.',englishVoiceUnavailable:'High-quality American male narration is not available on this device.',voiceListUnavailable:'No suitable narration voice is currently available on this device.',gotIt:'Got it',helpersWithYou:'Your Danco team helpers are with you',profileUntimedReminder:'Take your time—this section is not timed.',timedReminder:'Your selected-language helper has the visual focus.',fullscreenOn:'Full-screen app view active.',fullscreenOff:'Full-screen app view closed.',fullscreenIos:'App view expanded. For a completely borderless iPhone experience, add the assessment to your Home Screen.',
  ...
  function setLanguage(lang){
    ...
    updateVoiceStatus(settings.lang);
    renderSetupAccess();
  }
  ...
  const VOICE_PREFERENCES = {
    en:{
      target:'en-US',
      approvedNames:['evan','aaron','nathan','alex','daniel','matthew','tom','microsoft guy','microsoft davis','microsoft christopher','microsoft andrew','microsoft eric','microsoft roger','microsoft steffan','google us english male'],
      blocked:['reed','rocko','eddy','fred','grandpa','ralph','albert','bad news','bahh','bells','boing','bubbles','cellos','good news','hysterical','jester','junior','kathy','organ','princess','samantha','superstar','trinoids','whisper','wobble','zarvox']
    },
    es:{
  ...
  function spanishVoiceScore(voice){
    const profile=VOICE_PREFERENCES.es;
    ...
    if(!locale.startsWith('es')) return -10000;
    let score=locale===normaliseLocale(profile.target)?180:45;
  ...
  function preferredSpanishVoice(voices=refreshSpeechVoices()){
    // Restore the original automatic Spanish selection used before manual voice saving was introduced.
    return voices
      .map((voice,index)=>({voice,index,score:spanishVoiceScore(voice)}))
      .filter(candidate=>candidate.score>-10000)
      .sort((a,b)=>b.score-a.score||a.index-b.index)[0]?.voice||null;
  }
  function approvedEnglishMaleVoice(voices=refreshSpeechVoices()){
    const profile=VOICE_PREFERENCES.en;
    const candidates=voices
      .filter(voice=>normaliseLocale(voice.lang)==='en-us')
      .map((voice,index)=>{
        const identity=voiceIdentity(voice);
        const nameIndex=profile.approvedNames.findIndex(token=>identity.includes(token));
        const blocked=profile.blocked.some(token=>identity.includes(token));
        const premium=/enhanced|premium|natural|neural|studio|siri/.test(identity);
        // Apple compact/novelty voices are what the recording demonstrated. Do not play them.
        if(blocked||nameIndex<0||(!premium&&/com\.apple|apple/.test(identity))) return {voice,index,score:-10000};
        let score=1000-(nameIndex*24);
        if(premium) score+=300;
        if(/microsoft|google/.test(identity)) score+=140;
  ...
  function preferredSpeechVoice(language){
    ...
    return language==='es'?preferredSpanishVoice(voices):approvedEnglishMaleVoice(voices);
  }
  function updateVoiceStatus(language=settings.lang){
    const status=$('narration-voice-status');
    if(!status) return;
    const lang=language==='es'?'es':'en';
    const voice=preferredSpeechVoice(lang);
    if(voice){
      status.classList.remove('warning');
      status.textContent=`${t('voiceReady')} ${voice.name} · ${voice.lang}`;
    }else{
      status.classList.add('warning');
      status.textContent=lang==='en'?t('voiceInstallRequired'):t('voiceListUnavailable');
    }
  }
  function initialiseSpeechVoices(){
    if(!('speechSynthesis' in window)) return;
    refreshSpeechVoices();
    const onVoicesChanged=()=>{refreshSpeechVoices();updateVoiceStatus(settings.lang);};
    ...
    setTimeout(()=>updateVoiceStatus(settings.lang),220);
  }
  ...
  function speakWhenVoicesReady(text,requestId,attempt){
    ...
    if(!voices.length&&attempt<8){
      speechRetryTimer=setTimeout(()=>speakWhenVoicesReady(text,requestId,attempt+1),140+(attempt*80));
      return;
    }
    ...
    if(!voice){
      ...
      updateVoiceStatus(language);
      return;
    }
    // Cancel first, then create/speak the utterance after a short delay. This avoids iOS reusing a stale voice.
    speechSynthesis.cancel();
    setTimeout(()=>{
      if(requestId!==speechRequestId) return;
      const utterance=new SpeechSynthesisUtterance(String(text||''));
      utterance.voice=voice;
      utterance.lang=voice.lang||VOICE_PREFERENCES[language].target;
      // Preserve the Spanish delivery that was previously approved. English uses the same measured pace with neutral pitch.
      utterance.rate=.9;
  ...
  function openHelp(){ updateVoiceStatus(settings.lang); $('help-drawer').classList.add('open'); $('help-backdrop').classList.add('open'); $('help-drawer').setAttribute('aria-hidden','false'); speak(`${t('helpTitle')}. ${t('helpDefault')}`,{remember:false}); }
  ...
  function init(){
    ...
    if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=19.0.0').catch(()=>{}));
  }
