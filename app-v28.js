...
function initialiseInstallWelcome() {
  const welcome = $('#installWelcome');
  if (!welcome || isStandaloneApp()) return;
  welcome.hidden = false;
  const ios = isIOSDevice();
  const iosSteps = $('#iosInstallSteps');
  const nativeButton = $('#nativeInstallButton');
  const hint = $('#installBrowserHint');
  if (ios) {
    iosSteps.hidden = false;
    hint.hidden = isSafariBrowser();
  } else {
    nativeButton.hidden = false;
    nativeButton.disabled = true;
    nativeButton.textContent = 'Preparing install…';
  }
  $('#continueBrowserButton').addEventListener('click', () => {
    welcome.hidden = true;
    $('#screenRoot')?.focus();
  });
  nativeButton?.addEventListener('click', async () => {
    ...
    if (result.outcome === 'accepted') welcome.hidden = true;
    installPrompt = null;
...
function initialiseInstall() {
  window.addEventListener('beforeinstallprompt', (event) => {
    ...
    const nativeButton = $('#nativeInstallButton');
    if (nativeButton) { nativeButton.hidden = false; nativeButton.disabled = false; nativeButton.textContent = 'Install Trakway Trivia'; }
  });
