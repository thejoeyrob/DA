importScripts('./narration-manifest.js?v=23.0.0');

const CACHE_NAME = 'danco-assessment-v23-shared-review-20260904';
const CORE_ASSETS = [
  './','./index.html?v=23.0.0','./app.css?v=23.0.0','./app.js?v=23.0.0','./narration-manifest.js?v=23.0.0','./manifest.webmanifest?v=23.0.0',
...
const NARRATION_ASSETS=['./narration-en.mp3?v=23.0.0','./narration-es.mp3?v=23.0.0'];
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    event.respondWith(fetch(event.request).then(response=>{const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(event.request,copy));return response;}).catch(()=>caches.match('./index.html?v=23.0.0')));
    return;
  }
